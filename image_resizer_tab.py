...
def create_gradient(width: int, height: int, c1_hex: str, c2_hex: str, direction: str) -> Image.Image:
    """Generates a smooth RGB gradient background."""
...
def compute_collage_slots(W: int, H: int, count: int, layout: str) -> list:
    """Computes bounding boxes [x, y, w, h] for collage layouts."""
    slots = []
...
def render_resizer_canvas(
    ...
    """Renders the composition onto a Pillow canvas."""
    # 1. Base Canvas
    if use_gradient:
...
def render_image_resizer_tab(config: dict):
    """Main Streamlit view for Image Resizer & Collage studio."""
    ...
    with bot_c1:
        filename_input = st.text_input("Filename:", value=f"image_{datetime.now().strftime('%Y%m%d_%H%M%S')}")

    with bot_c2:
        format_choice = st.selectbox("Format:", ["WebP (.webp)", "JPG (.jpg)", "PNG (.png)"])
    ...
    clean_name = re.sub(r'[^a-zA-Z0-9-_\s]', '', filename_input).strip().replace(' ', '_')
    ...
    with bot_c4:
        if st.button("🚀 Send to Publisher", type="primary", width="stretch"):
            ...
            st.session_state["story_img_name"] = clean_name
    ...
    with bot_c5:
        if st.button("🌐 Send to Social Manager", width="stretch"):
            ...
            st.success(f"✅ Sent to Social Media Manager as `{clean_name}`!")
