...
def render_social_manager_tab(config: dict):
    ...
    with opt_col1:
        ...
        selected_provider = st.selectbox("AI Platform:", provider_options, index=default_index, key="social_ai_platform")

    with opt_col2:
        chk_youtube = st.checkbox("Generate YouTube SEO", value=True)

    with opt_col3:
        # Default to prefix sent from resizer
        preset_pfx = st.session_state.get("social_custom_prefix", "")
        custom_prefix = st.text_input("Custom Card Prefix (Optional):", value=preset_pfx, placeholder="e.g. breaking_news", key="social_pfx_input")
    ...
    if do_card:
        if not headline.strip():
            ...
            with st.spinner("🎨 Rendering social media card via Playwright..."):
                ...
                # Use custom prefix if provided, otherwise fallback
                if custom_prefix.strip():
                    card_filename = f"{custom_prefix.strip()}_{timestamp}.png"
                else:
                    card_filename = f"news_card_{timestamp}.png"
    ...
    if do_seo:
        if not headline.strip():
            ...
            if not api_key:
                st.error(f"❌ Missing API Key for {selected_provider}. Please configure it in Settings or Streamlit Secrets.")
            else:
